...
\obs Dos consecuencias inmediatas de la definición elemental. Si $A$ y $B$ son independientes
($P(A \cap B) = P(A) P(B)$), entonces $P(A \mid B) = P(A)$: saber que ocurrió $B$ no aporta
información sobre $A$. Y si $\{B_1, B_2, \dots\}$ es una partición de $\Omega$ con
$P(B_i) > 0$, los conjuntos $\{A \cap B_i\}$ parten a $A$, de modo que
$$ P(A) = \sum_i P(A \cap B_i) = \sum_i P(A \mid B_i) \, P(B_i)
\qquad \text{(probabilidad total)}, $$
y despejando de $P(B_k \mid A) \, P(A) = P(A \cap B_k) = P(A \mid B_k) \, P(B_k)$ se obtiene la
\textbf{fórmula de Bayes}:
$$ P(B_k \mid A) = \frac{P(A \mid B_k) \, P(B_k)}{\sum_i P(A \mid B_i) \, P(B_i)}. $$
La esperanza condicional respecto de una $\sigma$-álgebra generalizará este promedio ponderado
sobre los elementos de una partición.
...
\begin{definicion}{Submartingala y supermartingala}{submartingala}
Un proceso $(X_n)$ adaptado e integrable es una \emph{submartingala} si
$$ E(X_n \mid \widetilde{\mathcal{F}}_{n-1}) \ge X_{n-1} \qquad \forall n \ge 1, $$
y una \emph{supermartingala} si vale la desigualdad opuesta,
$E(X_n \mid \widetilde{\mathcal{F}}_{n-1}) \le X_{n-1}$.
\end{definicion}

\obs En el lenguaje del juego: una martingala es un juego justo, una submartingala uno
favorable al jugador y una supermartingala uno desfavorable. Un proceso es martingala si y
solo si es a la vez sub y supermartingala, y las esperanzas $E(X_n)$ resultan crecientes en el
caso sub y decrecientes en el caso super. Si $(X_n)$ es martingala y $\varphi$ es convexa con
$\varphi(X_n)$ integrable, entonces $(\varphi(X_n))$ es submartingala: por Jensen condicional
(Teorema \ref{teo:jensen_cond}),
$E(\varphi(X_n) \mid \widetilde{\mathcal{F}}_{n-1}) \ge
\varphi(E(X_n \mid \widetilde{\mathcal{F}}_{n-1})) = \varphi(X_{n-1})$. En particular
$(|X_n|)$ y $(|X_n|^p)$, con $p \ge 1$, son submartingalas.
...
\obs (\emph{Órdenes de magnitud}.) La razón de que el término de segundo orden sobreviva se ve
comparando, sobre una partición de $[a, b]$ de norma $\delta$, el tamaño de los cuatro
diferenciales que aparecen al desarrollar Taylor:
\begin{align*}
dt &: & \sum_k (t_k - t_{k-1}) &= b - a, \\
(dW)^2 &: & \sum_k (W_{t_k} - W_{t_{k-1}})^2 &\xrightarrow{L^2} b - a
&& \text{(variación cuadrática)}, \\
(dt)^2 &: & \sum_k (t_k - t_{k-1})^2 &\le \delta \, (b - a) \longrightarrow 0, \\
dt \, dW &: & \sum_k |W_{t_k} - W_{t_{k-1}}| \, (t_k - t_{k-1}) &\approx \sqrt{\delta} \,
(b - a) \longrightarrow 0,
\end{align*}
donde en la última línea se usó que $E((W_{t_k} - W_{t_{k-1}})^2) = t_k - t_{k-1}$, de modo que
el incremento típico es $|W_{t_k} - W_{t_{k-1}}| \approx \sqrt{t_k - t_{k-1}} \le \sqrt{\delta}$.
Es decir: $dt$ y $(dW)^2$ son del mismo orden y contribuyen, mientras que $(dt)^2$ y $dt \, dW$
se anulan en el límite. En el cálculo clásico —donde en lugar de $(dW)^2$ aparece $(dt)^2$— el
término de segundo orden desaparece y el cambio de variables queda con una sola integral.
